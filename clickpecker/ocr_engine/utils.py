...
def _is_parent(box1, box2):
    return ((box1.x <= box2.x) and (box1.y <= box2.y)
...
def add_box_paddings(box_arr,
                     max_width,
                     max_height,
                     paddings_arr=[5, 5, 10, 10]):
    x, y, w, h = box_arr
...
def rebase_box(boxes, dx, dy, c_w, c_h, o_w, o_h):
    ...
    return [
        ContentBox(box.content, position)
...
def abs_to_rel(boxes, width, height):
    ...
    rel_positions = (Box(pos.x / width, pos.y / height, pos.w / width,
                         pos.h / height) for pos in positions)
    return [
